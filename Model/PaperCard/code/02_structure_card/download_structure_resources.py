...
from ske.structure.roles import best_summary_facet_role, contains_evidence, infer_importance, infer_role_from_sentence
...
def parse_pubmed_file(path: Path, split_name: str, limit_docs: int | None) -> list[StructuredDocument]:
    ...
    with path.open("r", encoding="utf-8", errors="ignore") as handle:
        for line in handle:
            ...
            section, role, importance = map_pubmed_label(raw_label)
            current.append(
                SentenceRecord(
                    text=sentence,
                    section=section,
...
def map_pubmed_label(label: str) -> tuple[str, str, float]:
    mapping = {
        "BACKGROUND": ("intro", "background", 0.45),
        "OBJECTIVE": ("intro", "objective", 0.82),
        "METHODS": ("method", "process", 0.62),
        "RESULTS": ("experiment", "result", 0.88),
        "CONCLUSIONS": ("conclusion", "finding", 0.78),
    }
    return mapping.get(label.upper(), ("intro", "background", 0.35))
            ...
            for sentence in split_sentences(str(paragraph)):
                ...
                sentences.append(
                    SentenceRecord(
                        ...
                        importance_label=infer_importance(sentence, role, evidence),
...
def aclsum_record_to_document(record: dict[str, Any], split_name: str, idx: int) -> StructuredDocument:
    ...
    sentences: list[SentenceRecord] = []
    for sentence in section_document(str(record.get("document") or "")):
        ...
        sentences.append(
            SentenceRecord(
                ...
                role=role,
                evidence_label=None,
                importance_label=infer_importance(sentence.text, role, confidence),
